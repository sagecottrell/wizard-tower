...
@export var width_left: int:
	get:
		return int(data.size_left);
	set(value):
		data.size_left = value;

var prev_right: float = 1;
var current_right: float = 1;
@export var width_right: int:
	get:
		return int(data.size_right);
	set(value):
		data.size_right = value;
	
var width: int:
	get:
		return int(data.size_right + data.size_left);

func _ready():
	# start out at the right size, we don't spawn in at the default size
	var w = data.size_right + data.size_left;
	current_left = data.size_left;
	current_right = data.size_right;
...
func do_scale(delta: float) -> void:
	var mat: StandardMaterial3D = background.get_active_material(0);
	if current_right != data.size_right and grow_timer_right < grow_time:
		var f = ease(grow_timer_right / grow_time, easing);
		current_right = (data.size_right - prev_right) * f + prev_right;
		if grow_timer_right + delta >= grow_time:
			current_right = data.size_right;
			prev_right = data.size_right;
			grow_timer_right = 0;
		else:
			grow_timer_right += delta;
	if current_left != data.size_left:
		var f = ease(grow_timer_left / grow_time, easing);
		current_left = (data.size_left - prev_left) * f + prev_left;
		if grow_timer_left + delta >= grow_time:
			current_left = data.size_left;
			prev_left = data.size_left;
			grow_timer_left = 0;
		else:
