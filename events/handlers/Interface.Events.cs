...
public static partial class InterfaceEvents {    
    ...
    public static bool TryHiding(InterfaceHidingEvent pre, [NotNullWhen(true)] out InterfaceHidedEvent? e) {
        e = null;
        if (OnHiding(pre).IsAllowed) {
            e = pre.Into();
            return true;
        }
        return false;
    }    
    public static bool TryShowing(InterfaceShowingEvent pre, [NotNullWhen(true)] out InterfaceShowedEvent? e) {
        e = null;
        if (OnShowing(pre).IsAllowed) {
            e = pre.Into();
            return true;
        }
        return false;
