...
public static partial class TowerEvents {    
    ...
    public static bool TryResourceChanging(TowerResourceChangingEvent pre, [NotNullWhen(true)] out TowerResourceChangedEvent? e) {
        e = null;
        if (OnResourceChanging(pre).IsAllowed) {
            e = pre.Into();
            return true;
        }
        return false;
